...
# Process data: calc logarithm
myData <- process_log(raw_data)

# Process data: replace -Inf with 0 (NOTE: MUST CONFIRM THIS FOR RIGHT LOGIC)
myData[is.infinite(myData)] <- 0
...
### unrestricted cons
vecm_l1_co_tsD <- VECM(myData, lag = 1, include = "const", estim = "ML")
vecm_l3_co_tsD <- VECM(myData, lag = 3, include = "const", estim = "ML")

vecm_l1_co_var <- ca.jo(myData, K = 2, ecdet = "none", spec = "transitory")
vecm_l3_co_var <- ca.jo(myData, K = 4, ecdet = "none", spec = "transitory")

### restricted cons
vecm_l1_LRco_tsD <- VECM(myData, lag = 1, LRinclude = "const", estim = "ML")
vecm_l1_LRco_var <- ca.jo(myData, K = 2, ecdet = "const", spec = "transitory")

vecm_l3_LRco_tsD <- VECM(myData, lag = 3, LRinclude = "const", estim = "ML")
vecm_l3_LRco_var <- ca.jo(myData, K = 4, ecdet = "const", spec = "transitory")

### restricted trend
vecm_l1_LRtr_tsD <- VECM(myData, lag = 1, LRinclude = "trend", estim = "ML")
vecm_l1_LRtr_var <- ca.jo(myData, K = 2, ecdet = "trend", spec = "transitory")

vecm_l3_LRtr_tsD <- VECM(myData, lag = 3, LRinclude = "trend", estim = "ML")
vecm_l3_LRtr_var <- ca.jo(myData, K = 4, ecdet = "trend", spec = "transitory")
...
vecm_Rrest_tsD <- VECM(myData, lag = 1, include = "const", estim = "ML", beta = R)
...
var_l1_co_tsD <- lineVar(myData, lag = 1, include = "const", beta = 2)
var_l1_tr_tsD <- lineVar(myData, lag = 1, include = "trend", beta = 2)
var_l1_bo_tsD <- lineVar(myData, lag = 1, include = "both", beta = 2)
var_l1_no_tsD <- lineVar(myData, lag = 1, include = "none", beta = 2)

var_l3_co_tsD <- lineVar(myData, lag = 3, include = "const", beta = 4)
var_l3_tr_tsD <- lineVar(myData, lag = 3, include = "trend", beta = 4)
var_l3_bo_tsD <- lineVar(myData, lag = 3, include = "both", beta = 4)
var_l3_no_tsD <- lineVar(myData, lag = 3, include = "none", beta = 4)

var_l1_co_var <- VAR(myData, p = 1, type = "const")
var_l1_tr_var <- VAR(myData, p = 1, type = "trend")
var_l1_bo_var <- VAR(myData, p = 1, type = "both")
var_l1_no_var <- VAR(myData, p = 1, type = "none")

var_l3_co_var <- VAR(myData, p = 3, type = "const")
var_l3_tr_var <- VAR(myData, p = 3, type = "trend")
var_l3_bo_var <- VAR(myData, p = 3, type = "both")
var_l3_no_var <- VAR(myData, p = 3, type = "none")
